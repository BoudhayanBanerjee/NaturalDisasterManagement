				...
				if(data){
					...
					if(Earthquakes == 1)
					{
						...
						var endDate = now.getUTCFullYear()+'-'+mm+'-'+
										...
										now.getUTCSeconds();
						var startDate = parseInt(now.getUTCDate())-1;
						startDate = now.getUTCFullYear()+'-'+mm+'-'+
						                startDate+'T'+now.getUTCHours()+':'+now.getUTCMinutes()+':'+
										now.getUTCSeconds();
						url_event = 'http://earthquake.usgs.gov/fdsnws/event/1/query?format=geojson&starttime='+startDate+'&endtime='+endDate;
